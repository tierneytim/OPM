...
for i  =1:S.division
    ...
    pa.faces = f;
    g = gifti(pa);
    [~,Di]=spm_mesh_neighbours(g,1);
    medVertexSpacing = median(Di(:));  
end

m2 = 'Median point seperation of solution space  is %3.3f mm.';
mesg = sprintf(m2,medVertexSpacing);
disp(mesg)
...
disp(mesg)
m2 = 'Final sd of nearest neighbour  is %3.3f mm.';
mesg = sprintf(m2,std(ms2s));
disp(mesg)
...
function [result]=cross_product(V1,V2)
 result=[(V1(2)*V2(3) - V2(2)*V1(3)),(V1(3)*V2(1) - V2(3)*V1(1)), (V1(1)*V2(2) - V2(1)*V1(2))];
end