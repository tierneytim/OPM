...
%   S.trigNN      - NN interpolation on TRIG channels [deafault: 0]
%
...
if ~isfield(S, 'trigNN'),       S.trigNN = 0;    end
...
%-The new downsampled M/EEG dataset
%--------------------------------------------------------------------------
Dnew = fsample(Dnew, S.fsample_new);

%-nearest neighbour on TRIG channels if requested
%--------------------------------------------------------------------------
if(S.trigNN)
    fprintf('%-40s: %30s\n','NN downsampling on TRIG channels',spm('time')); 
    trigInds = indchantype(D,'TRIG');
    x=D.time();
    xq=Dnew.time();
    for i =1:ntrials(D)
